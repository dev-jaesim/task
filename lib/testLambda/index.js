...
function extractCoordinates(response) {
    ...
    for (let feature of response.features) {
        if (feature.properties.address !== baseAddress) {
            result.test = 'Multiple features from NSW_Geocoded_Addressing_Theme have different addresses.'
            result.testResult = 'Fail';
            return result;
        }
        
        if (!feature.geometry || !feature.geometry.coordinates) {
            result.test = 'A feature from NSW_Geocoded_Addressing_Theme is missing coordinates.'
            result.testResult = 'Fail';
            return result;
        }
...
async function fetchAdministrativeBoundaries(coordinates, url, propName, errorMessage) {
    ...
    const errorMessages = {
        'suburbname': 'Different suburb names for the same address.',
        'districtname': 'Different district names for the same address.'
    };
