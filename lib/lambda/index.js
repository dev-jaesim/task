...
// Address cannot include any special characters.
...
// Create a custom error class to return consistent error messages in the same format.
...
// Validate the address parameter from users.
...
// Raise an error if no features are found in the response.
...
// Retrieve location information based on the input address.
async function fetchAddressData(address) {
    const url = `${config.ADDRESS_POINT_URL}${encodeURIComponent(address)}'`;
    return callApi(url);
}

// Extract ID(s) to return.
function extractIds(data) {
    let ids = [];

    if (data.features && Array.isArray(data.features)) {
        ids = data.features.map(feature => feature.id);
    }
...
// If there are multiple features in the response, it extracts all coordinates from the response.
function extractCoordinates(response) {
    let coordinates = [];
    const baseAddress = response.features[0].properties.address;

    for (let feature of response.features) {
        // If there are multiple features that have different addresses, it returns an error.
        if (feature.properties.address !== baseAddress) {
            raiseError(400, ERROR_MESSAGES.ADDRESSES_NOT_SAME);
        }
        
        // If there are feature that doesn't have coordinates, it returns an error.
...
// Send HTTP calls to NSW_Administrative_Boundaries_Theme.
...
// It extracts suburb names and state electoral district names.
// If there are multiple coordinates based on the user input, it sends an HTTP call with each coordinate and compare the response.
// If there are responses, it returns an error. 
...
exports.handler = async (event) => {
    try {
        // If there is no query string from the request, it returns an error.
        ...
        return {
            statusCode: 200,
            body: JSON.stringify({
                ids: extractIds(nswGeocodeAddressingThemeResponse),
                location: coordinates,
                suburbName: suburb,
                stateElectoralDistrictName: stateElectoralDistrict
            }),
        };        
    } catch (error) {
        console.error(error);
